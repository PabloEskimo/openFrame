...
abstract class BaseData extends Base  {
	...
	public function __construct( $id = false ) {
		...
		if(array_search('construct', get_class_methods($chrClass)) !== false){
			call_user_func_array(array($this, 'construct'), array());
		}
	 ...
	 * @return true;
	 */
	final public function setLoaded( $blnLoaded ) {
				
		$this->blnLoaded = $blnLoaded;
	...
	/**
	 * Checks if an identical record exists - and returns the ID if it does
	 * @param 
	 * @return $idRecord;
	 */
	final public function exists() {
		...
		foreach($arrFields as $chrField => $chrValue){
			...
			$chrQueryFields .= "$chrPrefix $chrField = ?\n";
		...
		if(sizeof($arrResults) > 0){
			return $arrResults[0]["id$chrClass"];
		} else {
			return false;
		}
	...
	/**
	 * Gets an array of all record names keyed by ID 
	 * @param 
	 * @return $arrRecords
	 */
	public static function get_names(  ) {
		...
		$chrQuery = "SELECT id$chrClass, chrName FROM $chrClass ORDER BY id$chrClass ASC";
		...
		$arrSearch = array();
		foreach($objQuery->fetchAll() as $intCount => $arrRow){
			$arrSearch[(int) $arrRow[0]] = $arrRow[1];
		}
	...
	final public static function __callStatic($chrMethod, $arrArguments) {
		...
		switch($chrMethodPrefix){
			...
			case 'find_by_':
				...
				alert("Unable to search $chrClass records by $chrProperty as this field doesn't exist");
	...
	final private function createTable(  ) {
		...
		$arrFieldPrefixes = array(
			...
			'str' => 'text',
			'obj' => 'text',
			'int' => 'int(1) NOT NULL DEFAULT 0',
			'bln' => 'int(1) NOT NULL DEFAULT 0',
			'id' => 'int(1)',
		);
